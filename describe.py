...
if (len(sys.argv) > 1):
    try:
        with open(sys.argv[1], newline='') as csvfile:
            ...
            print("\nmin", end="\t\t")
            for t in tab:
                print(f"{round(t[0],6):>12}",end="\t")
            print("\n25%", end="\t\t")
            for t in tab:
                print(f"{round(t[round(len(t)/4)],6):>12}",end="\t")
            print("\n50%", end="\t\t")
            for t in tab:
                print(f"{round(t[round(len(t)/2)],6):>12}",end="\t")
            print("\n75%", end="\t\t")
            for t in tab:
                print(f"{round(t[round(len(t)/3)*2],6):>12}",end="\t")
            print("\nmax", end="\t\t")
            for t in tab:
                print(f"{round(t[-1],6):>12}",end="\t")
